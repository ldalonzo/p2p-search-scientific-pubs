...
class PDFPaperParserHtml(PDFPaperParser):
    ...
    def feed(self, pdfFileURL):
        ...
        ## TRUE for enable reference analysis VERY SLOW!!!!!!!
        ## FALSE for disable
        #metadata = htmlparser.getMetadata(False)
    ...
    def convertPdfToHtml(self, pdfFileURN):
        ...
        try:
            ...
            #-i ignore images
            #-q don't print any messages of errors
            timeStart = datetime.datetime.now()
            ...
            timeJob = datetime.datetime.now() - timeStart
            timeJobString = "%d.%d" % (timeJob.seconds, timeJob.microseconds)
            #print "[%s] pdftohtml tooks %s seconds. It returned exitCode=%d" % ( threading.currentThread().getName(), timeJobString, exitCode )
            if exitCode == 0:
                #fsock = codecs.open(tempHtmlFile, 'r', 'utf-8')
                 #CHANGED 25/02/2009 problems with some unknown char codes
...
class Pdf2HtmlParser(sgmllib.SGMLParser):
    ...
    def __init__(self):
        ...
        #self.__currentContextProperties = {}
    ...
    def filterAuthors(self, text):
        ...
        if (not re.search('[0-9]+|@|&', text) and (not re.search(regexp_ban_words, text)) ):
            ...
            text = re.sub(',[\s]*$', '', text) #removes any comma from the end of string
            ...
            for item in filtered_list_1:
                ...
                if len(single_words)<2 or len(single_words)>3:
                    #print "It doesn't appear as an author because the single words are not the between 2 and 3"
                    return None
                elif ((not re.search('^[A-Z]', single_words[0])) or (not re.search('^[A-Z]', single_words[len(single_words)-1]))):
                    #print "It doesn't appear as an author"
                    return None
        ...
        #print "REFUSED! It contains number or @"
        return None
    ...
    def getMetadata(self, ReferenceAnalysis=True):
        ...
        #print "[DEBUG_getMetadata] References: %s" % self.__rawReferences
        #filterCitations(self.__rawReferences)
        
        def __filterAbstractText(text):
            #text = re.sub('^[\s]*Abstract[\s]*', '', text, re.IGNORECASE)
        ...
        def __normalizePunctuation(text):
            text = re.sub('\*$', '', text) # remove possible * char at the end of string
            text = re.sub(' :', ':', text)
            text = re.sub(' ,', ',', text)
            return text
        ...
        if len(self.__rawReferences)>0:
            ...
            #metadata['bibliography'] = self.__rawReferences
    ...
    def start_br(self, attrs):
        self.__breakLine += ' '
        #print "[DEBUG-start_br] A CAPO! %s" % attrs
    ...
    def handle_data(self, text):
        #print "[DEBUG-handle_data] ANALYZING _%s_" % text 
        #print "[DEBUG-handle_data] ANALYZING _%d_" % len(text)
        
        def __checkTopPositionForTitleLine():
            #print "Checking top position for the title... it is %d" % self.__currentTextProperties['top']
        ...
        def __checkTopPositionForAuthorLine():
            #print "Checking top position for the author... it is %d" % self.__currentTextProperties['top']
            maxSpaceBetweenTitleAndAuthors = 180
            if self.__titleTextProperties.has_key('top') and self.__currentTextProperties['top']>0:
                #print "[DEBUG-__checkTopPositionForAuthorLine] __currentTextProperties['top'] = %s" % self.__currentTextProperties['top']
                #print "[DEBUG-__checkTopPositionForAuthorLine] __titleTextProperties['top'] = %s" % self.__titleTextProperties['top']
        ...
        def __canBeATitleLine(text):
            #print "Can be a title? _%s_" % text
            if __checkTopPositionForTitleLine():
                ...
                #in caso di e.g. 4 font diversi con la stessa dimensione la funzione
                #precedente falliva
        ...
        def __canConsiderTitleCompleted(text):
            #print "Analyzing _%s_" % text
            #print self.__currentTextProperties['font']
            ##Se si omette la prima condizione è possibile che un titolo a due linee venga tagliato
            ##e la seconda parte del titolo venga riconosciuta come un autore
        ...
        def __canBeAnAuthorLine(text):
            #print "[DEBUG-canBeAnAuthorLine] Analyzing: %s" % text
            if __checkTopPositionForAuthorLine():
                ...
                if self.__titleTextProperties.has_key('font'):
                    ...
                    #print "[DEBUG-canBeAnAuthorLine] titleFontSize = %d, currentFontSize = %d" % (titleFontSize, currentFontSize)
        ...
        def __canBeTheAbstract(text):
            if self.__current_page < self.MAX_ABSTRACT_PAGE:
                #print "[DEBUG__canBeTheAbstract] Analyzing _%s_" % text
            ...
            if self.__current_page > 0 and re.search('[\w]+', text): #TODO
                if self.__currentTextProperties.has_key('medium_font-size') and self.__currentTextProperties.has_key('font'):
                    ...
                    if difference > threshold:
                        #print "[DEBUG__canBeANewSection] The difference between current font and medium font size is %d" % difference
                        #print "[DEBUG__canBeANewSection] Text is _%s_" % text
                        return True
            return False
        ...
        def __canStopAbstract(text):
            #print "[DEBUG-STOP_ABSTRACT] Analyzing: %s\n" % text
            ...
            if self.__abstractTextProperties.has_key('medium_font-size') and self.__currentTextProperties.has_key('font'):
                fontSizeThreshold = 1
                #difference = int(self.__fontsFace[self.__currentTextProperties['font']] - self.__abstractTextProperties['medium_font-size'])
                ...
                if re.search('\.[\s]*$', self.__abstract) and difference > fontSizeThreshold:
                    #print "[DEBUG-STOP_ABSTRACT] The new font face differs by %d points from abstract text. Abstract can considered retrieved." % difference
                    return True
                elif difference > 2 * fontSizeThreshold:
                    #print "[DEBUG-STOP_ABSTRACT] The new font face differs by %d points from abstract text, but maybe the abstract has been cut because I didn't find the stop mark. Abstract will be consider retrieved." % difference
                    return True
                #print "[DEBUG-STOP_ABSTRACT] currentTextProperties = %d, abstractTextProperties = %d" % (self.__fontsFace[self.__currentTextProperties['font']],self.__abstractTextProperties['medium_font-size'])
                #print "[DEBUG-STOP_ABSTRACT] The new font face differs only by %d points from abstract text. Abstract can't consider retrieved yet." % difference
                
            if re.search('\.[\s]*$', self.__abstract) and re.search('^[\s]*Key[\s]*word|Introduction|^[\s]*Index[\s]+terms', text, re.IGNORECASE):
                #print "[DEBUG-STOP_ABSTRACT] Founded one of the following keywords: Introduction, Keyword or Index Terms"
                return True
            ...
            if self.__currentRetrievalStage == 'IDLE':
                ...
                if self.__current_page == 0:
                    ...
                    if __canBeTheAbstract(text) and len(self.__abstract)==0:
                        ...
                        self.__breakLine = ''#azzera lo stack; necessario(?) per i ritorni a capo <br>
                ...
                #print "[DEBUG__getFutureState] currentPage is %d" % self.__current_page
                return 'IDLE'
        ...
        def __takeAction(text):
            ...
            def __removeHypensAndNormalizeWhiteSpaces(stackText, rawText, removeLineBreak=True):
                ...
                if re.search('[a-z]+-[\s]*$', stackText):#added [\s]*
                    if re.search('^[\s]+', rawText):
                        rawText = re.sub('^[\s]+', '', rawText)
                    #print "A CAPO! _%s_" % rawText
            ...
            if self.__currentRetrievalStage == 'TITLE':
                ...
                if mediumFontSize:
                    ...
                    #print "MFS = %d" %  self.__abstractTextProperties['medium_font-size']
                #print self.__abstract
                
            elif self.__currentRetrievalStage == 'REFERENCES':
                """Current retrieval stage is REFERENCES
                """
                #self.__rawReferences = __removeHypensAndNormalizeWhiteSpaces(self.__rawReferences, text)
                #self.__rawReferences += " %s" %text
        ...
        if self.__currentTextProperties.has_key('font') and self.__current_page>0:
            ...
            #print "[DEBUG] CurrentRetrievalStage is %s" % self.__currentRetrievalStage
            #if not self.__currentTextProperties['top'] == 'unknown':
            #    print "(%d)\t%s" % (self.__currentTextProperties['top'], text)
            __takeAction(text)
        #print "[DEBUG-handle_data] finished!"
    ...
    def handle_comment(self, text):
        if re.search('\.ft[0-9]+', text):
            ...
            #self.__currentContextProperties['font_size'] = self.__fontsFace[self.__top3LargestFonts[0]]
