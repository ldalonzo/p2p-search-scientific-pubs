...
#                except Exception, exception:#KeyboardInterrupt:
#                    print "[%s] network error" % threading.currentThread().getName()
#                    print "[", threading.currentThread().getName(), "]", " Exception arguments ", exception.args
#                    print "[", threading.currentThread().getName(), "]", " Exception  ", exception
            self.__peerStatus.setRecommendation(recommendations)

##############queryTerms = ['networks', 'overlays']
##############self.__recommendations['leo'] = self.__index.queryTitle(queryTerms, numOfItemToRecommend)
...
class BuddyCast(threading.Thread):
    ...
    def run(self):
        ...
        while(self.__activated):
            ...
            ###### DEBUG
            #stemmedProfile = {'uno':0.123, 'due':0.102, 'tre':0.0932}     # leo
            #stemmedProfile = {'uno':0.123, 'tre':0.102, 'quattro':0.0932} # johan
...
###################
            ...
            ###### DEBUG
            #peerProfile = {'uno':0.123, 'due':0.102, 'tre':0.0932}     # leo
            #peerProfile = {'uno':0.123, 'tre':0.102, 'quattro':0.0932} # johan
            
#            try:
#                print "\n[%s] posting %s's profile to the superpeer@%s" % (threading.currentThread().getName(), name, self.__superPeer)                
#                self.__superPeer.postPeerProfile(name, peerProfile)
...
class Clusterizer(threading.Thread):
    ...
    def run(self):
        ...
        while(self.__activated):
            ...
            try:
                #result = cl.computeClusterCentroid(titleDictionary)
                ...
                #print "[%s] setting PeerProfile..." % threading.currentThread().getName()
                ...
                #print "[%s] PeerProfile successfully set!" % threading.currentThread().getName()
...
class Indexer(threading.Thread):
    ...
    def directoriesToIndex(self):
        ...
        #dirs.append('/media/disk/workspace/experiments/20090601/dataset/leo')
        #dirs.append('/media/disk/workspace/experiments/20090601/dataset/frank')
    ...
    def run(self):
        ...
        self.__index.buildCitGraphAndAnalyze() ##FIXME
        #self.__index.logAllTermByDocMatrix()
        #self.__index.logAbstractTermByDocMatrix() ##FIXME
        while (self.__active):
...
class Peer:
    ...
    def getTasteBuddiesDjango(self):
        ...
        ##DEBUGGING VALUES
        #self.__buddies['leo'] = ('10.250.11.250', '3128', '0.25')
        #self.__buddies['johan'] = ('10.250.11.249', '3127', '0.35')        
    ...
    def getRecommendationDjango(self):
        ...
        #for tasteBuddy in recommendations.keys():
        #    djangoRet[tasteBuddy] = presentation.encodeResultAsXml_IDF(recommendations[tasteBuddy])        
        #return djangoRet
    ...
    def setRecommendation(self, recommendations):
        ...
        #queryTerms = ['networks', 'overlays']
        #self.__recommendations['leo'] = self.__index.queryTitle(queryTerms, numOfItemToRecommend)
        #self.__recommendations['johan'] = self.__index.queryTitle(queryTerms, numOfItemToRecommend)
        
        self.__condition.release()
        
    def getPeerProfileAndModify(self):
        self.__condition.acquire()
        #print "[%s] Now you can modify the peer profile!" % threading.currentThread().getName()
        return self.__peerProfile
    
    def committPeerProfileModifications(self):
        self.__peerProfileUpdated.set()
        #print "[%s] Committing peer profile modification. Releasing the lock" % threading.currentThread().getName()
        self.__condition.release()
        return True
    ...
    def setTasteBuddies(self, tasteBuddiesList):
        ...
        #self.__recommendations = {} #reset recommendations
        self.__buddies = {}
        for buddyTuple in tasteBuddiesList:
            (buddyName, buddySim, buddyAddress, buddyPort) = buddyTuple
            #(buddyName, buddySim, buddyProxy) = buddy
            #print "_%s_" % buddyTuple
...
class PeerServer(threading.Thread):
    ...
    def run(self):
        ...
        while(self.__activated): 
            #(item1, item2) = self.__peerServer.get_request()
            #print item1
            #print item2
...
class superPeerConnection____DEPRECATED():
