...
class Dictionary(Persistence):
    ...
    def addTerm(self, term, document, frequency):
        ...
        ### Removed for boost the performance
        ### Use committ() 
    ...
    def updateIDF(self, docsInTheCollection):
        ...
        self.save(self.__pickledFileName, [self.__dictionary])
            #print "%s:         \t\t%s - (%d, %f)" % (term, docID, postingFile[docID][0], postingFile[docID][1])
...
#            postingFile = self.__dictionary[term]
#            for docID in postingFile.keys():
#                if not debugDIC.has_key(docID):
#                    debugDIC[docID] = []
#                debugDIC[docID].append( postingFile[docID][1] )
#                
#        for docID in debugDIC:
#            docVector = debugDIC[docID]
#            print docVector
#            acc = float(0)
#            for component in docVector:
#                acc += math.pow(component, 2)
#            print "%s has a norm of %f" % (docID, acc)
