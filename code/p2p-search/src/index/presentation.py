...
def encodeResultAsXml(resultSet = []):
    ...
    for paper in resultSet:
        ...
        if hasattr(paper, 'abstract'):
            ...
            #if len(abstractText)>maxAbstractLength:
        ...
        numOfCitations = "%d" % len(paper.getCitingPapers())##FIXME is not a string!!!
...
def encodeResultAsXml_IDF(resultSet = []):
    ...
    for tuple in resultSet:
        ...
        #print "URL of the paper _%s_ is _%s_" % (paper.title, resURL)
        ...
        pagerank = xmlDoc.createElement('pagerank')
        pagerank.appendChild(xmlDoc.createTextNode(pagerankScoreText))
