...
def filter_punctuation_from_words(list_of_words):
    ...
    for filtered_word in list_of_words:
        if re.search('\.$', filtered_word):
        ...
        if re.search('\:$', filtered_word):
            filtered_word = re.sub('\:$', '', filtered_word)
        if re.search('^\([a-z]*\)$', filtered_word):
            filtered_word = re.sub('^\(', '', filtered_word)
            filtered_word = re.sub('\)$', '', filtered_word)
        if re.search('^\(', filtered_word):
            filtered_word = re.sub('^\(', '', filtered_word)
...
def filterWords(textToIndex):
    ...
    return [word for word in list_of_words_to_index if len(word)>0]
