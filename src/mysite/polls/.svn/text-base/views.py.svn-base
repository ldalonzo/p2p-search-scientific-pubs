...
#def hello(request):
#    http = "<html><body>Hello World! %s</body></html>" % datetime.datetime.now() 
...
def startSearch(request):
    ...
    for term in centroidTagClouds.keys():
        #print "%s,%s" % (term, centroidTagClouds[term])
    ...
    #print "largest is %.3f" % largestWeight
    #print "smallest is %.3f" % smallestWeight
    ...
    if deltaWeight>0:
        ...
        for term in orderedSet:
            tag = {}
            #fontSize = (deltaFontSize/deltaWeight) * centroidTagClouds[term]
            ...
            #tagCloud[term] = math.ceil( target )
            tagCloud.append(tag)
        
        #print "%.4f --> %.3f (%d)" % (centroidTagClouds[term], target, math.ceil(target))
...
def query(request):
    ...
    #http = "<html><body><h1>Response</h1><br/>%s</body></html>" % response_utf8
    #latest_poll_list = ['mele', 'pere', 'banane']
    ...
    #output = ', '.join(list)
    return HttpResponse(t.render(c))
    #return HttpResponse(response_utf8)
...
def searchCloud(request, keyword):
    ...
    #t = loader.get_template('polls/resultsIDF.html') ####OLD VERSION of the template
...
def getCitedBy(request, paperURN):
    ...
    t = loader.get_template('polls/results.html')
    c = Context({'resultSet': results,
                 #'query': 'citing papers of ' + paperURN,
                 'lunghezza' : lunghezza})
