...
class BuddyCast(threading.Thread):
    ...
    def __run_DEBUG(self):
        #self.__peerProxy.setBuddies(buddyList)
        ...
        for buddyData in self.__buddyList:
            #(buddyName, buddyProxy, buddySim) = buddyData
            (buddyName, buddyAddress, buddyPort, buddySim) = buddyData
            print "\t (%.4f) %s@%s" % (buddySim, buddyName, buddyProxy)
            #print "\t _%s_" % buddyData
        time.sleep(3)
        #print "[%s] bye!" % threading.currentThread().getName()
    ...
    def run(self):
        ...
        if DEBUG:
            ...
            for buddyData in self.__buddyList:
                ...
                #dataToBeTransmitted.append(buddyProxy) ###FUNZIONA!!
                dataToBeTransmitted.append( (buddyName, buddySim, buddyAddress, buddyPort) )
                
            #print dataToBeTransmitted
            try:
...
class BuddyBuilder(threading.Thread):
    ...
    def __calculateFriendships(self, similarityMatrix, matrixBase, maxNumOfBuddies=2):
        ...
        #print "matrixBase is _%s_" % matrixBase        
        #tasteBuddies = {}
    ...
    def __spreadOutFriendships__DEPRECATED(self, tasteBuddiesAdjList, matrixBase):
        
        peersInTheNetwork = self.__network.getPeers()
        
        peerBuddiesID = 0
        for peerBuddies in tasteBuddiesAdjList:
            
            peerName = matrixBase[peerBuddiesID]
            if peersInTheNetwork.has_key(peerName):
                
                (peerAddress, peerPort, proxyPeerServer) = peersInTheNetwork[peerName]
                
                print "\n%s (_%s_) is friend with:" % (peerName, proxyPeerServer)
                ...
                for buddyID in peerBuddies:
                    ...
                    if peersInTheNetwork.has_key(peerName):
                        ...
                        buddiesProxies.append(proxyBuddyServer)
                        print "\t%s proxy is _%s_" % (buddyName, proxyBuddyServer)
                    else:
                        print "the buddy %s seems seems to be no longer reachable" % buddyName
                        
                if len(buddiesProxies)>0:
                    print "trying contacting peer %s for transmittig _%s_" % (peerName, buddiesProxies)
                else:
                    print "%s has no taste buddies" % peerName        
                
            else:
                print "unable to contact the peer .. maybe it left the network..."
    ...
    def __computeFriendshipsAndSpreadOut(self, similarityMatrix, matrixBase, maxNumOfBuddies=2):
            ...
            if peersInTheNetwork.has_key(peerName):
                ...
                for buddyID in peerBuddies:
                    ...
                    if peersInTheNetwork.has_key(peerName):
                        ...
                        #buddiesProxies.append((buddyName, proxyBuddyServer, similarityValue)) ####DEPRECATED
                        buddiesProxies.append((buddyName, buddyAddress, buddyPort, similarityValue)) ####DEPRECATED
                        #print "\t%s proxy is _%s_" % (buddyName, proxyBuddyServer)
                    else:
                ...
                #    print "trying contacting peer %s for transmitting _%s_" % (peerName, buddiesProxies)
                else:
                    #print "[%s] peer _%s_ has no taste buddies." % (threading.currentThread().getName(), peerName)
        ...
        for peerName in spreadBuddyDict.keys():
            ...
            # The content of the dict must be a tuple like the following:
            #(buddyName, buddyAddress, buddyPort, buddySim)
            buddyPeers = spreadBuddyDict[peerName]
            
            #print "[%s] creating contacting thread for %s (%s)" % (threading.currentThread().getName(), peerName, name)
    ...
    def __printTasteBuddiesAdjacencyMatrix(self, tasteBuddiesAdjList, matrixBase):
        #print "[%s] Taste buddies adjacency list:" % threading.currentThread().getName()
        #print "\t_%s_" % tasteBuddiesAdjList
    ...
    def run(self):
        while(True):
            ...
            #tasteBuddiesAdjList = self.__calculateFriendships(similarityMatrix, matrixBase, 2)
            
            #self.__printTasteBuddiesAdjacencyMatrix(tasteBuddiesAdjList, matrixBase)
            
            ###
            ###  {'peerName':[listOfBuddiesProxyServer]}
            
            #print "[%s] spreading out data..." % threading.currentThread().getName()
            
            #self.__spreadOutFriendships(tasteBuddiesAdjList, matrixBase)
...
class P2PNetwork():
    ...
    def __init__(self):
        ...
        #self.__superPeerAddress = 'localhost'
        #self.__superPeerPort = 0
    ...
    def getPeerProfilesMatrix(self):
        #print "[%s] waiting for update" % threading.currentThread().getName()
        self.__peerProfileMatrixUpdated.wait()
        self.__peerProfileMatrixUpdated.clear()
        #print "[%s] changed detected! wait for condition..." % threading.currentThread().getName()
    ...
    #def setSuperPeerAddress(self, SPAddress):
    #    self.__superPeerAddress = SPAddress
        
    #def setSuperPeerPort(self, SPPort):
    #    self.__superPeerPort = SPPort
        
    #def getSuperPeerAddress(self):
    #    return self.__superPeerAddress
    
    #def getSuperPeerPort(self):
    #    return self.__superPeerPort
    ...
    def registerPeer(self, peerName, peerAddress):
        #print "peerName = _%s_" % peerName 
        #print "peeraddress = _%s_" % peerAddress
        ...
        #print "peerAddress = %s" % peerAddress
        #print "peerPort = %s" % peerPort
        ...
        #proxyPeerServer.allow_none = True
        #proxyPeerServer.allow_reuse_address = True####???????????????????
    ...
    def __calculatePeerProfileSimilarities(self):
        ...
        self.__peerProfileMatrixBase.sort()##Added for sort the keys (i.e. peerNames)
        ...
        #print "[%s] setting event!" % threading.currentThread().getName()
        self.__peerProfileMatrixUpdated.set()
        #print "[%s] holding the condition for a while..." % threading.currentThread().getName()
        #time.sleep(2)
        
        self.__condition.release()
        #print "[%s] condition released!" % threading.currentThread().getName()
    ...
    def __calculateSimilarities(self, profileA, profileB):
        ...
        #print "profileA (%.4f): _%s_" % (profileANorm, profileA)
        ...
        #print "profileB (%.4f): _%s_" % (profileBNorm, profileB)
...
class SuperPeerServer(threading.Thread):
    ...
    def run(self):        
        try:
            ...
            #self.__buddyCastProcess = BuddyBuilder(self.__status)
            #self.__buddyCastProcess.start()
            while(not self.__shutdown):
...
if __name__ == '__main__':
    ...
    if OPTION == 2:
        ...
        #print "[%s] Hello!" % (threading.currentThread().getName())
        ...
        try:
            pass
            #superPeer.join()
            #while(True):#trovare una soluzione migliore!
            #    time.sleep(5000)
        except KeyboardInterrupt:
    ...
    ### DEBUG    
        ...
        peerProfile = {'uno':0.123, 'due':0.102, 'tre':0.0932}
        
        net.registerPeer(peerName, peerAddress)
        net.postPeerProfile(peerName, peerProfile)
        
        time.sleep(7)
        print "\n____________________"
        peerName = 'johan'
        peerAddress = 'localhost'
        peerProfile = {'uno':0.123, 'tre':0.102, 'quattro':0.0932}
        ...
        peerName = 'tamas'
        peerAddress = 'localhost'
        peerProfile = {'due':0.123, 'quattro':0.102, 'cinque':0.0932}
        ...
        peerName = 'dave'
        peerAddress = 'localhost'
        peerProfile = {'quattro':0.323, 'cinque':0.102, 'sei':0.0732}
        ...
        peerName = 'martin'
        peerAddress = 'localhost'
        peerProfile = {'sette':0.323, 'otto':0.102, 'nove':0.0732}
