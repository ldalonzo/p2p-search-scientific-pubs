...
sub Help {
  ...
  print STDERR "       $progname [-qE] [-r <rankfile> -n <num>] filename(s)...\n";
...
our ($opt_q, $opt_v, $opt_h, $opt_r, $opt_n, $opt_E);
...
while (<$fh>) {
  ...
  if (/^\s*$/) {
    ...
    if ($variant eq "") {
      print "  <entry no=\"$line\">\n";
      print "    <variant no=\"0\" confidence=\"$confidence\">" . $buf . "</$lastTag></variant>\n";
      print "  </entry>\n";
      $line++;
    } else {
	...
	$line++;
      ...
      $buf2 .= "    <variant no=\"$variant\" confidence=\"$confidence\">" . $buf . "</$lastTag></variant>\n";
    }
